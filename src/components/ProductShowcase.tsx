...
const ProductShowcase = () => {
  ...
  const renderProductCard = (product: DiamondProduct) => <div key={product.id} className="bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-lg transition-shadow duration-300 cursor-pointer w-full" onClick={() => handleProductClick(product)}>
      {/* Product image */}
      <div className="relative aspect-square overflow-hidden">
        ...
        <div className="absolute top-1 md:top-4 left-1 md:left-4 flex flex-col space-y-1 md:space-y-2">
          {product.discount_percentage && product.discount_percentage > 0 && <Badge className="bg-red-500 text-white text-xs font-semibold px-1 md:px-2 py-1">
              {product.discount_percentage}% OFF
            </Badge>}
          {product.ships_today && <Badge className="bg-blue-500 text-white text-xs font-semibold px-1 md:px-2 py-1">
              SHIPS TODAY
            </Badge>}
        </div>

        {/* Size options - hidden on mobile */}
        {product.sizes && product.sizes.length > 0 && <div className="absolute bottom-1 md:bottom-4 left-1 md:left-4 flex flex-wrap gap-1 hidden md:flex">
            {product.sizes.slice(0, 3).map((size, index) => <Badge key={index} className="bg-gray-800 text-white text-xs px-2 py-1">
                {size}
              </Badge>)}
          </div>}
      </div>
      ...
      <div className="p-2 md:p-4">
        ...
        <div className="flex items-center mb-2 md:mb-4">
          ...
          {product.original_price && <span className="ml-1 md:ml-2 text-sm md:text-lg text-gray-400 line-through">
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8">
            ...
            <div className="flex space-x-0 border-b border-gray-200">
              {['BEST SELLERS', 'NEW ARRIVALS'].map(tab => <Button key={tab} variant="ghost" className="px-4 py-2 rounded-none border-b-2 border-transparent text-gray-600">
                  {tab}
                </Button>)}
            </div>
          </div>
          ...
          {products.length === 0 ? <div className="text-center py-12">
              ...
              {isMobile ? <Carousel className="w-full">
                  <CarouselContent className="-ml-2 md:-ml-4">
                    {products.map(product => <CarouselItem key={product.id} className="pl-2 md:pl-4 basis-4/5">
