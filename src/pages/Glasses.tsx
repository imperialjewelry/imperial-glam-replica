...
const Glasses = () => {
  ...
  const { products, filters, loading } = useProductFilters('glasses_products');
  ...
  // Get frame styles from products for filter
  const frameStyles = products.reduce((acc: { [key: string]: number }, product) => {
    if (product.frame_style) {
      acc[product.frame_style] = (acc[product.frame_style] || 0) + 1;
    }
    return acc;
  }, {});
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="bg-gray-50 py-12 px-8">
          <div className="max-w-7xl mx-auto text-center">
            ...
            <p className="text-lg text-gray-600">
              Premium Designer Glasses Collection
            </p>
          </div>
        </div>
      )}
      ...
      <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'}`}>
        ...
        {!isMobile && (
          <div className="w-64 bg-white p-6 border-r border-gray-200 min-h-screen">
            ...
            {frameStyleOptions.length > 0 && (
              <FilterSection
                title="FRAME STYLE"
                items={frameStyleOptions}
                selectedItems={[]}
                onToggle={() => {}}
              />
            )}
      ...
      {/* Product Modal */}
      {selectedProduct && (
        <GlassesProductModal product={selectedProduct} onClose={() => setSelectedProduct(null)} />
      )}
