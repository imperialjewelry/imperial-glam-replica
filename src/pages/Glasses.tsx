...
const Glasses = () => {
  ...
  const fetchProducts = async () => {
    const {
      data,
      error
    } = await supabase.from('glasses_products').select('*').order('created_at', {
      ascending: false
    });
    if (error) {
      ...
      <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'}`}>
        ...
        <div className={`flex-1 ${isMobile ? 'py-4 px-4' : 'py-8 px-8'}`}>
          ...
          <div className={`grid ${isMobile ? 'grid-cols-2' : 'grid-cols-4'} gap-4`}>
            {products.map(product => <div key={product.id} className="bg-white rounded-lg border hover:shadow-lg transition-shadow" onClick={() => setSelectedProduct(product)}>
