...
const Watches = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('watch_products').select('*').order('created_at', {
        ascending: false
      });
  ...
  const productTypes = [{
    name: "Presidential Watches",
    count: allProducts.filter(p => p.product_type === "Presidential Watches").length
  }, {
    name: "Ships Today",
    count: allProducts.filter(p => p.ships_today).length
  }, {
    name: "Bust Down Watches",
    count: allProducts.filter(p => p.product_type === "Bust Down Watches").length
  }, {
    name: "Skeleton Watches",
    count: allProducts.filter(p => p.product_type === "Skeleton Watches").length
  }];
  const colors = [{
    name: "Yellow Gold",
    count: allProducts.filter(p => p.color === "Yellow Gold").length
  }, {
    name: "White Gold",
    count: allProducts.filter(p => p.color === "White Gold").length
  }, {
    name: "Rose Gold",
    count: allProducts.filter(p => p.color === "Rose Gold").length
  }, {
    name: "Black Gold",
    count: allProducts.filter(p => p.color === "Black Gold").length
  }];
  const materials = [{
    name: "925 Silver",
    count: allProducts.filter(p => p.material === "925 Silver").length
  }, {
    name: "Solid Gold",
    count: allProducts.filter(p => p.material === "Solid Gold").length
  }, {
    name: "Brass",
    count: allProducts.filter(p => p.material === "Brass").length
  }];
  const gemstones = [{
    name: "Moissanite",
    count: allProducts.filter(p => p.gemstone === "Moissanite").length
  }, {
    name: "VVS Diamond Simulants (CZ)",
    count: allProducts.filter(p => p.gemstone === "VVS Diamond Simulants (CZ)").length
  }, {
    name: "VVS Moissanite",
    count: allProducts.filter(p => p.gemstone === "VVS Moissanite").length
  }, {
    name: "VVS Moissanites",
    count: allProducts.filter(p => p.gemstone === "VVS Moissanites").length
  }];
  const diamondCuts = [{
    name: "Round Cut",
    count: allProducts.filter(p => p.diamond_cut === "Round Cut").length
  }, {
    name: "Baguette",
    count: allProducts.filter(p => p.diamond_cut === "Baguette").length
  }, {
    name: "Emerald Cut",
    count: allProducts.filter(p => p.diamond_cut === "Emerald Cut").length
  }, {
    name: "Baguette Cut",
    count: allProducts.filter(p => p.diamond_cut === "Baguette Cut").length
  }, {
    name: "Heart Cut",
    count: allProducts.filter(p => p.diamond_cut === "Heart Cut").length
  }, {
    name: "Marquise Cut",
    count: allProducts.filter(p => p.diamond_cut === "Marquise Cut").length
  }, {
    name: "Oval Cut",
    count: allProducts.filter(p => p.diamond_cut === "Oval Cut").length
  }, {
    name: "Pear Cut",
    count: allProducts.filter(p => p.diamond_cut === "Pear Cut").length
  }, {
    name: "Princess Cut",
    count: allProducts.filter(p => p.diamond_cut === "Princess Cut").length
  }];
  const watchTypes = ["PRESIDENTIAL WATCHES", "BUST DOWN WATCHES", "SKELETON WATCHES"];
        ...
        <div className="space-y-3">
          {productTypes.map(type => <div key={type.name} className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <Checkbox id={`desktop-${type.name}`} checked={selectedProductTypes.includes(type.name)} onCheckedChange={checked => handleProductTypeChange(type.name, checked as boolean)} />
      ...
      <div className="mb-8">
        ...
        <div className="space-y-3">
          {colors.map(color => <div key={color.name} className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <Checkbox id={`desktop-${color.name}`} checked={selectedColors.includes(color.name)} onCheckedChange={checked => handleColorChange(color.name, checked as boolean)} />
      ...
      <div className="mb-8">
        ...
        <div className="space-y-3">
          {materials.map(material => <div key={material.name} className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <Checkbox id={`desktop-${material.name}`} checked={selectedMaterials.includes(material.name)} onCheckedChange={checked => handleMaterialChange(material.name, checked as boolean)} />
  ...
  const renderMobileFilters = () => showFilters && <div className="bg-white border rounded-lg mb-6 overflow-hidden">
        ...
        <Collapsible open={openSections.productType} onOpenChange={() => toggleSection('productType')}>
          ...
          <CollapsibleContent className="p-4 border-b">
            <div className="space-y-3">
              {productTypes.map(type => <div key={type.name} className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    <Checkbox id={type.name} checked={selectedProductTypes.includes(type.name)} onCheckedChange={checked => handleProductTypeChange(type.name, checked as boolean)} />
        ...
        <Collapsible open={openSections.color} onOpenChange={() => toggleSection('color')}>
          ...
          <CollapsibleContent className="p-4 border-b">
            <div className="space-y-3">
              {colors.map(color => <div key={color.name} className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    <Checkbox id={color.name} checked={selectedColors.includes(color.name)} onCheckedChange={checked => handleColorChange(color.name, checked as boolean)} />
        ...
        <Collapsible open={openSections.material} onOpenChange={() => toggleSection('material')}>
          ...
          <CollapsibleContent className="p-4 border-b">
            <div className="space-y-3">
              {materials.map(material => <div key={material.name} className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    <Checkbox id={material.name} checked={selectedMaterials.includes(material.name)} onCheckedChange={checked => handleMaterialChange(material.name, checked as boolean)} />
        ...
        <Collapsible open={openSections.gemstone} onOpenChange={() => toggleSection('gemstone')}>
          ...
          <CollapsibleContent className="p-4 border-b">
            <div className="space-y-3">
              {gemstones.map(gemstone => <div key={gemstone.name} className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    <Checkbox id={gemstone.name} checked={selectedGemstones.includes(gemstone.name)} onCheckedChange={checked => handleGemstoneChange(gemstone.name, checked as boolean)} />
        ...
        <Collapsible open={openSections.diamondCut} onOpenChange={() => toggleSection('diamondCut')}>
          ...
          <CollapsibleContent className="p-4">
            <div className="space-y-3">
              {diamondCuts.map(cut => <div key={cut.name} className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    <Checkbox id={cut.name} checked={selectedDiamondCuts.includes(cut.name)} onCheckedChange={checked => handleDiamondCutChange(cut.name, checked as boolean)} />
  ...
  return <div className="min-h-screen bg-white">
          ...
          <div className="max-w-7xl mx-auto">
            <div className="text-center">
              ...
              <div className="flex justify-center space-x-8 text-sm text-gray-500">
                {watchTypes.map((type, index) => <span key={index} className="border-r border-gray-300 pr-8 last:border-r-0">
                    {type}
                  </span>)}
              </div>
            </div>
      ...
      {/* Mobile Hero Section */}
      {isMobile && <section className="bg-gray-50 py-8 px-4">
          <div className="max-w-sm mx-auto">            
            <div className="text-center">
              ...
              <div className="flex justify-center space-x-4 mb-6 text-xs">
                {watchTypes.map((type, index) => <span key={index} className="text-gray-500 border-r border-gray-300 pr-4 last:border-r-0">
                    {type}
                  </span>)}
              </div>
            </div>
      ...
      <div className={`flex ${isMobile ? 'flex-col' : 'flex-row'}`}>
        ...
        <div className={`flex-1 ${isMobile ? 'py-4 px-4' : 'py-8 px-8'}`}>
          ...
          <div className={`grid ${isMobile ? 'grid-cols-2' : 'grid-cols-4'} gap-4`}>
            {filteredAndSortedProducts.map(product => <div key={product.id} className="bg-white rounded-lg border hover:shadow-lg transition-shadow cursor-pointer" onClick={() => handleProductClick(product)}>
                ...
                <div className="relative aspect-square overflow-hidden rounded-t-lg">
                  ...
                  {product.sizes && product.sizes.length > 0 && <div className="absolute bottom-2 left-2 flex flex-wrap gap-1">
                      {product.sizes.slice(0, 2).map((size, index) => <Badge key={index} variant="secondary" className="text-xs">
                          {size}
                        </Badge>)}
                      {product.sizes.length > 2 && <Badge variant="secondary" className="text-xs">
      ...
      {/* Watch Product Modal */}
      {selectedProduct && isModalOpen && <WatchProductModal product={selectedProduct} onClose={() => {
